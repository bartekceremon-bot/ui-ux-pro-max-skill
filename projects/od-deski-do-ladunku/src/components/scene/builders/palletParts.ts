...
BOTTOM_BOARDS.forEach((board, i) => {
  PARTS.push({
    ...
    explodeDir: [0, -1.4, board.z === 0 ? 0 : Math.sign(board.z) * 0.5],
...
BLOCK_X.forEach((x, xi) => {
  BLOCK_Z.forEach((z, zi) => {
    PARTS.push({
      ...
      rest: [x, Y_BLOCK, z],
      explodeDir: [Math.sign(x) * 0.45, -0.35, Math.sign(z) * 0.45],
...
BLOCK_X.forEach((x, i) => {
  PARTS.push({
    ...
    rest: [x, Y_STRINGER, 0],
    explodeDir: [Math.sign(x) * 0.55, 0.35, 0],
    spawn: [x * 2.2, 1.9, 1.8],
...
TOP_BOARDS.forEach((board, i) => {
  PARTS.push({
    ...
    explodeDir: [0, 1.25 + i * 0.06, board.z * 0.6],
